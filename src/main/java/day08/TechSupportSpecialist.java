...
public abstract class TechSupportSpecialist {
    ...
    public void setNext(TechSupportSpecialist next) {
        this.next = next;
    }
    ...
    public void handleRequest(ServiceRequest request) {
        // TODO
        if (isResponsibleFor(request.getCustomer()) && isQualifiedToHandle(request.getSubject())) {
            resolveRequest(request);
        } else if (next != null) {
            next.handleRequest(request);
        } else {
            request.setResponse("Please call at later time!");
        }
    }
